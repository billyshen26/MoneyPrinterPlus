...
with folder_container:
    st.subheader(tr("Video Source Folder"))
    st.text_input(
        label=tr("Video Folder Path"),
        placeholder=tr("Please input video folder path"),
...
with bg_music_container:
    ...
    with llm_columns[0]:
        st.text_input(
            label=tr("Background Music Dir"),
            placeholder=tr("Input Background Music Dir"),
...
with video_generator:
    st.button(label=tr("Generate Video Button"), type="primary", on_click=generate_simple_merge,
